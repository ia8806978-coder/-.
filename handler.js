...
//─๋︩︪─๋︩︪─๋︩︪─๋︩︪──๋︩︪─๋︩︪─๋︩︪─๋︩︪─๋︩︪─
...
if (m.money) m.reply(+m.money + ` HARONCoins ${await tr("usados")} 🪙`) 
}
break
}}} catch (e) {
console.error(e)
...
if (!m.fromMem && m.text.match(/(@966559958322|@966559958322|هارون|بوت|:v)/gi)) {
...
if (media === 'texto') {
this.sendMessage(id, { text: text, contextInfo: { forwardedNewsletterMessageInfo: { newsletterJid: '120363355261011910@newsletter', serverMessageId: '', newsletterName: 'PAIN 𝙱𝙾𝚃 ✨' }, forwardingScore: 9999999, isForwarded: true, mentionedJid: [user], externalAdReply: { showAdAttribution: true, renderLargerThumbnail: true, thumbnail: apii.data, title: [wm, ' ' + wm + '😊', '🌟'].getRandom(), containsAutoReply: true, mediaType: 1, sourceUrl: [nna, nna2, nnntt, yt].getRandom() } } }, { quoted: fkontak2 });
}
if (media === 'audio') {
this.sendMessage(id, { audio: { url: vn }, contextInfo: { forwardedNewsletterMessageInfo: { newsletterJid: '120363419326701988@newsletter', serverMessageId: '', newsletterName: 'PAIN 𝙱𝙾𝚃 ✨' }, forwardingScore: 9999999, isForwarded: true, mentionedJid: [user], externalAdReply: { mediaType: 1, previewType: "PHOTO", thumbnail: apii.data, title: `乂 🎤┇≡ ◡̈⃝🎼•⪼ مـرحـبـا يـا برو 乂`, body: [wm, ' ' + wm + '😊', '🌟'].getRandom(), showAdAttribution: true, renderLargerThumbnail: true, sourceUrl: [nna, nna2, nnntt, yt].getRandom() } }, ptt: true, mimetype: 'audio/mpeg', fileName: `error.mp3` }, { quoted: fkontak2 });
}}}}
...
if (msg) return conn.sendMessage(m.chat, {
  ...
  contextInfo: {
    ...
    forwardedNewsletterMessageInfo: {
      newsletterJid: '120363419326701988@newsletter',
      serverMessageId: '',
      newsletterName: 'PAIN 𝙱𝙾𝚃'
    },
    externalAdReply: {
      mediaUrl: null,
      mediaType: 1,
      ...
      body: wm,
      previewType: 0,
      thumbnail: await fetch(await conn.profilePictureUrl(m.sender, 'image')).then(res => res.buffer()).catch(() => fs.readFileSync('./media/Menu.jpg'))
    }
  }
}, { quoted: m });
}
